		...
		ArkInventory.Lib.DewDrop:Open( this,
			...
			"children", function( level, value )
				...
				if level == 1 then
					...
					local title = ( ArkInventory.Global.Mode and ArkInventory.Global.Mode.VaultContext == "personal" ) and "Personal Bank" or GUILD_BANK
					ArkInventory.Lib.DewDrop:AddLine(
						"text", string.format( "%s: %s", title, string.format( GUILDBANK_TAB_NUMBER, bag_id ) ),
						"isTitle", true,
						"icon", bag.texture,
						"textHeight", 12
					)
					...
					if not ( ArkInventory.Global.Mode and ArkInventory.Global.Mode.VaultContext == "personal" ) then
					ArkInventory.Lib.DewDrop:AddLine(
	...
	if level == offset + 1 then
		...
		local fc = 0
			...
			for n, tp in ArkInventory.spairs( ArkInventory.db.realm.player.data, function(a,b) return a < b end ) do
				...
				local show = false
				local personalVault = ( loc_id == ArkInventory.Const.Location.Vault ) and ( ArkInventory.Global.Mode and ArkInventory.Global.Mode.VaultContext == "personal" )

				if loc_id ~= ArkInventory.Const.Location.Vault or personalVault then
