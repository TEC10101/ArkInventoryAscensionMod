...
function ArkInventory.BagEmpty( loc_id, bag_id )
	...
	local cbag = ArkInventory.BagID_Blizzard( loc_id, bag_id )
	if not cbag then
		return
	end
	...
	for _, dest_bag in pairs( ArkInventory.Global.Location[loc_id].Bags ) do

		local _, bt = GetContainerNumFreeSlots( dest_bag )

		if dest_bag ~= cbag and ( bt == 0 or bt == ct ) then

			for slot_id = 1, GetContainerNumSlots( dest_bag ) do

				if ( loc_id == ArkInventory.Const.Location.Bank ) and ( not ArkInventory.Global.Mode.Bank ) then
					return -- no longer at bank, abort
				end

				local h = GetContainerItemLink( dest_bag, slot_id )
				...
				if not h then
					...
					if h then
						...
						PickupContainerItem( dest_bag, slot_id )
						ClearCursor( )
					end
