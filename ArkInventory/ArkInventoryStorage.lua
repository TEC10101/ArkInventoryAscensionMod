	...
	for p, pd in pairs( ArkInventory.db.realm.player.data ) do
		...
		if player_id == nil or string.lower( p ) == string.lower( player_id ) then
			...
			if loc_id == nil or loc_id == ArkInventory.Const.Location.Vault then
				...
				pd.control[ArkInventory.Const.Location.Key] = false
				pd.control[ArkInventory.Const.Location.Vault] = false
        pd.control[ArkInventory.Const.Location.PersonalBank] = true
        pd.control[ArkInventory.Const.Location.RealmBank] = true
...
function ArkInventory:LISTEN_VAULT_ENTER( )
	...
	if not title and GuildBankFrame and GuildBankFrame.TitleText and GuildBankFrame.TitleText.GetText then
