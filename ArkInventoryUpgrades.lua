	...
	if ArkInventory.db.profile.option.version < upgrade_version then
		...
		for k, v in pairs( ArkInventory.db.profile.option.rule ) do
			if type( v ) == "table" then
				-- new structure { enabled=bool, usable=bool } - normalise enabled only
				v.enabled = v.enabled and true or false
				ArkInventory.db.profile.option.rule[k] = v
			elseif v then
...
function ArkInventory.ConvertOldOptions( )
	...
	for k, v in pairs( cat.data ) do
		if v.used then
			local rp = ArkInventory.db.profile.option.rule[k]
			if type( rp ) == "table" then
				v.enabled = rp.enabled and true or false
			else
				v.enabled = rp and true or false
			end
