...
-- helper functions for per-profile rule flags (enabled, usable)
local function RuleProfileGet( rid, create )
	...
	if type( rp ) ~= "table" then
		...
		if create then
			ArkInventory.db.profile.option.rule[rid] = rp
		end
	end
	return rp
end

function ArkInventory.RuleProfileGetEnabled( rid )
	local rp = ArkInventory.db.profile.option.rule[rid]
	if type( rp ) == "table" then
		return rp.enabled and true or false
	elseif rp == nil then
		return false
	else
		return rp and true or false
	end
end

function ArkInventory.RuleProfileSetEnabled( rid, enabled )
	local rp = RuleProfileGet( rid, true )
	rp.enabled = enabled and true or false
end
...
function ArkInventory.RuleProfileSetUsable( rid, usable )
	local rp = RuleProfileGet( rid, true )
	rp.usable = usable and true or false
end
...
function ArkInventory.RuleItemAdd( rulenum, h )
	...
	r["enabled"] = ArkInventory.RuleProfileGetEnabled( rulenum )
...
--	end
	...
	for k, d in pairs( ArkInventory.db.global.option.category[ArkInventory.Const.Category.Type.Rule].data ) do
		...
		if not ArkInventory.db.global.option.showdisabled and not ArkInventory.RuleProfileGetEnabled( k ) then
			ignore = true
		end

		if not ignore then
			local enabled = ArkInventory.RuleProfileGetEnabled( k )
			local usable = ArkInventory.RuleProfileGetUsable( k )
			local sort_group = usable and 0 or 1
			table.insert( t, {
				["sorted"]=format( "%1i %04i %04i", sort_group, d.order or 0, k ),
				["id"]=k,
				["enabled"]=enabled,
				["usable"]=usable,
				["order"]=d.order or 0,
...
function ArkInventory.Frame_Rules_Table_Usable_OnClick( self )
	...
	ArkInventory.RuleProfileSetUsable( id, self:GetChecked( ) )
